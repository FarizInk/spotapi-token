...
export const generateCode = async (context) => {
  ...
  if (value.get('username') === config().USERNAME && value.get('password') === config().PASSWORD) {
    ...
    context.response.redirect(`https://accounts.spotify.com/authorize?response_type=code&client_id=${config().CLIENT_ID}&scope=${encodeURI(scopes)}&state=${encodeURI(state)}&redirect_uri=${encodeURI(config().REDIRECT_URI_HOST + "/callback")}`);
  } else {
